...
from DuskyMusic import YouTube, app
from DuskyMusic.core.call import Dusky
from DuskyMusic.misc import SUDOERS, db
from DuskyMusic.utils.database import (is_active_chat,
...
from DuskyMusic.utils.formatters import seconds_to_min
from DuskyMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Dusky.pause_stream(chat_id)
        ...
        await Dusky.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Dusky.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Dusky.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Dusky.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Dusky.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
